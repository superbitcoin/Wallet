...
class UiRequest(object):
    ...
    def sendHeader(self, status=200, content_type="text/html", noscript=False, extra_headers=[]):
        ...
        if content_type != "text/html" and self.env.get("HTTP_REFERER") and self.isSameOrigin(self.getReferer(),
                                                                                              self.getRequestUrl()):
            headers.append(("Access-Control-Allow-Origin", "*"))  # Allow load font files from css

        if noscript:
            headers.append(("Content-Security-Policy",
                            "default-src 'none'; sandbox allow-top-navigation allow-forms; img-src 'self'; font-src 'self'; media-src 'self'; style-src 'self' 'unsafe-inline';"))
        ...
        if status in (200, 206) and cacheable_type:  # Cache Css, Js, Image files for 10min
            ...
            headers.append(
                ("Cache-Control", "no-cache, no-store, private, must-revalidate, max-age=0"))  # No caching at all
    ...
    def actionWrapper(self, path, extra_headers=None):
        ...
        if match:
            ...
            if (site and site.content_manager.contents.get("content.json") and (
        ...
        if "htm" in content_type and not header_noscript:  # Valid nonce must present to render html files
            ...
            if referer and path_parts:  # Only allow same site to receive media
                if not self.isSameOrigin(self.getRequestUrl(), self.getReferer()):
                    self.log.error(
                        "Media referrer error: %s not allowed from %s" % (self.getRequestUrl(), self.getReferer()))
        ...
        if match:  # Looks like a valid path
            ...
            if ".." in file_path or not os.path.dirname(os.path.abspath(file_path)).startswith(allowed_dir):
                ...
                return self.actionFile(file_path,
                                       header_length=False)  # Dont's send site to allow plugins append content
        else:  # Bad url
        ...
        if os.path.isfile(file_path):
            ...
            if send_header:
                ...
                if range:
                    ...
                    status = 200
                self.sendHeader(status, content_type=content_type, noscript=header_noscript,
    ...
    def formatError(self, title, message, details=True):
        ...
        if details:
            ...
            details["version_wallet"] = "%s r%s" % (config.version, config.rev)
            ...
            return """
                ...
                * { font-family: Consolas, Monospace; color: #333 }
                ...
                <h1>%s</h1>
                <h2>%s</h3>
                <h3>Please <a href="https://github.com/superbitcoin/SuperBitcoin/issues" target="_blank">report it</a> if you think this an error.</h3>
                <h4>Details:</h4>
                <pre>%s</pre>
